...
| `400` | 入力が不正な場合 |
| `401` | 未認証の場合 |
| `403` | 権限が足りない場合、または**人の操作だと確かめられなかった場合** |
...
| 項目 | 必須 | 内容 |
|---|---|---|
| `X-Recaptcha-Token`（見出し） | **本番では ○** | reCAPTCHA v3 の合図です。**行動の名前は `generate_prompt` です**（下記）。開発・テストでは、秘密鍵が設定されているときだけ求めます |
...
**投入する経路だけを守ります。** 閲覧の経路（一覧・取り出し）は求めません。理由は 3 つです。

1. **守る対象は「投入」です。** 閲覧は状態を変えず、枠も使いません。
2. **閲覧はすでに認証とプラン値で守られています。** 合図を重ねても、守りは増えません。
3. **画面を移るたびに Google へ問い合わせると、待ち時間が増え、第三者への送信も増えます。**

**合図は見出しで送ります。** 本文へ混ぜません。**合図は認証の材料であって、生成の入力ではありません。**

**本番では必ず求めます。** 開発とテストでは、秘密鍵（`RECAPTCHA_SECRET_KEY`）が設定されているときだけ求めます。**この分かれ方は環境だけで決まり、要求側の値では切り替わりません。**
