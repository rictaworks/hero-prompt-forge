...
export function NewRequestForm() {
  ...
  //
  // **開発サーバーでは、この画面がサーバーで描かれます。** 描かれた直後の
  // HTML には、まだ送信処理（`onSubmit`）が付いていません。組み上がる前に
  // 送信ボタンが押されると、ブラウザの既定の動き（素の GET 送信）が起き、
  // `/requests/new?...` へ遷移して入力していただいた内容が消えます
  // （実測で確認されました）。**マウントが完了するまで `disabled` を持たせ、
  // この隙を塞ぎます。**
