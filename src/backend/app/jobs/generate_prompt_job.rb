...
class GeneratePromptJob < ApplicationJob
  ...
  # **同じ生成リクエストの組み立ては、同時に 1 つだけです**（issue #181）。
  #
  # ## 起きていたことです（PR #176 のレビューで実測されました）
  #
  # 定時の拾い直し（`ReclaimPromptRequestsJob`）が置き去りの行を新しく
  # 投入した最中に、もとの回が待ち行列から戻ってくることがあります
  # （`executions` が 2 以上の投入し直しです）。`resumable?` は投入し直し
  # なら行の持ち時間を見ずに再開させますので、**2 つの投入が同時に組み立てへ
  # 進み、有償の呼び出しが二重にかかります。** あとから収める側が
  # `PromptOutput` の一意制約（`prompt_request_id` × `variation_no`）で
  # 落ち、待ち行列に理由の分からない失敗として残ります。
  #
  # ## Solid Queue の同時実行の制限で防ぎます（issue #181 の候補 1）
  #
  # **`with_lock`（`claimed?`）だけでは足りません。** あれは「行を読んでから
  # 書くまで」の錠であって、**2 つの投入が別々のタイミングで行を読み書き
  # する分には、どちらも順番に錠を取れてしまいます。** 二重の組み立てを
  # 防ぐには、**2 つ目の投入そのものを、1 つ目が終わるまで待ち行列で
  # 待たせる**必要があります。
  #
  # **掴み手の印を行へ持たせる案（候補 2）は採りません。** マイグレーションが
  # 要り、Solid Queue が既に持つ仕組みを自作で置き換えることになります。
  # `limits_concurrency` は Solid Queue 本体の機能で、車輪の再発明を
  # 避けられます。
  #
  # - `key`：生成リクエストの識別子です。**同じ識別子につき、進行中の
  #   投入は 1 つだけ**にします。
  # - `on_conflict: :block`：あとから来た投入を**破棄しません。** 待ち行列に
  #   残し、先の投入が終わり次第すぐに解放します（Solid Queue が先の投入の
  #   終了を検知した時点で解放する仕組みを持っています）。**破棄すると、
  #   もし先の投入が例外を上げずに落ちた場合、その回を拾い直す機会が
  #   1 つ失われます。**
  # - `duration`：`STALE_AFTER` に合わせます。**先の投入が例外を上げずに
  #   落ちた場合**（働き手ごと落ちた場合）は、上の「終了を検知した時点での
  #   解放」が起きません。その保険として、`STALE_AFTER` を越えたら
  #   強制的に解放します。組み立ての長さ（最大 60 秒）より長く、Solid
  #   Queue が仕事を戻す窓（およそ 5 分）より短い値です。**行の持ち時間の
  #   判定（`stale?`）と同じ理屈で選んでいます。**
  limits_concurrency to: 1,
                     key: ->(prompt_request_id) { prompt_request_id },
