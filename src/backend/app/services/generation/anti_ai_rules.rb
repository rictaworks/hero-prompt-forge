...
module Generation
  ...
  #     「同じ語の別の書き方」です
  #   - **語の形をそろえます。** 単数と複数、英国式と米国式のつづりは
  #     「同じ語の別の書き方」です（issue #136。手当ては WordForms が持ちます）
  ...
  class AntiAiRules
    ...
    #
    # **全角の英数字を半角へそろえます。** 日本語入力のままローマ字を打つと
    # 全角になります。`ＴＥＡＬ のアクセント` を取りこぼしていました。
    #
    # **語の形もそろえます。** 単数と複数、英国式と米国式のつづりは、
    # 語彙の問題ではなく書き方の問題です（issue #136）。そろえるのは語ごとで、
    # 語の並びは変えません。語の切れ目で見る性質は保たれます。
    def self.normalize(term)
      widened = term.to_s.unicode_normalize(:nfkc)

      WordForms.canonical(widened.downcase.gsub(WORD_SEPARATORS, ' ').strip)
    end
