...
module Generation
  ...
  #
  # **中身を検めます。** 位置ごとに 4 つの役割（余白・被写体・収まり・
  # 静けさ）がそろっていること、値がそのままプロンプトへ入れられる英文で
  # あることを確かめます。欠けたまま通すと、コピースペースの指定を欠いた案が
  # 出ます（requirements.md 4.2 が禁じています）。
  #
  # **この定義は版を持ちません。** 規則辞書（`rule_dictionaries`）と違い、
  # 変えると過去の生成が「どの内容で作られたか」を追えなくなります
  # （requirements.md 7.2）。**変更の頻度が低い前提です。** 運用で頻繁に
  # 触るようになった場合は、規則辞書と同じく版を持つ形へ移してください。
  class CopySpaceRules
    ...
    class << self
      ...
      # テストから読み直せるようにします。
      #
      # **本番の経路では使いません。** 読み直しは、定義を差し替えて検める
      # テストのためだけに要ります。
      def reset!
        @definition = nil
      end
