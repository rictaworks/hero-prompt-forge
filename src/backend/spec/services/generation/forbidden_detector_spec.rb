...
RSpec.describe Generation::ForbiddenDetector do
  ...
  # 助詞と複合名詞へ広げた分です（issue #134）。
  #
  # **通す側と止める側を対で固定します。** 検出側だけを追うと、通してよい
  # 文章まで止める向きへ痩せても気づけません。
  describe '助詞と複合名詞への広がり' do
    [
      ['助詞が「は」です', '他社のロゴは画面の中央に配置してください。'],
      ['複合名詞です', '他社のロゴ画像を背景に載せてください。'],
      ['副助詞が挟まります', '他社のロゴだけをそのまま使用します。'],
      ['副助詞が挟まります', '競合のロゴなどを並べて配置してください。'],
      ['修飾が12文字を超えます', '他社のロゴを、画面の右下の目立つ位置に小さく配置してください。']
    ].each do |reason, text|
      it "#{reason}：「#{text}」を差し戻します" do
        expect(detect(text)).to be_forbidden
      end
    end

    # **主語と目的語を取り違えません。**
    # 「他社のロゴが入った看板」の目的語は看板です。助詞に「が」を加えると、
    # 「他社のロゴが目印です」のような文まで巻き込みます。
    it '主語として現れる言い回しは、取りこぼしたまま残します' do
      expect(detect('他社のロゴが入った看板を背景に使います。')).not_to be_forbidden
    end

    it '見つかった語が読みやすい形になります' do
      result = detect('他社のロゴマークを大きく掲載してください。')

      expect(result.reasons.first.matched).to include('ロゴマーク')
    end
  end
